...
function score(item: Omit<RecommendationItem, 'selected'>, pref: UserPreference): number {
  ...
  if ('preferHiddenRoute' in pref && pref.preferHiddenRoute && item.popularity === 'hidden') s += 2;
  if ('acceptHotspot' in pref && pref.acceptHotspot && item.popularity === 'hot') s += 1;
...
function buildValidatedDay(selected: RecommendationItem[], pref: UserPreference, dayIndex: number): ItineraryItem[] {
  ...
  if (!hasFood) {
    ...
    if (foodFallback) items.splice(Math.min(1, items.length), 0, toItineraryItem(foodFallback, '中午', pref));
  }
  ...
  if (!hasExperience) {
    ...
    if (expFallback) items.unshift(toItineraryItem(expFallback, '上午', pref));
  }
  ...
  if (pref.staminaLevel === 'light') {
    return finalItems.filter((item) => durationToMinutes(item.estimatedDuration) <= 150).slice(0, 4);
  }
